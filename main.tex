...
% \section{Task 4 - Container Security Analysis}

\subsection{Tổng quan về Đánh giá Bảo mật}
...
\subsection{Phân tích Rủi ro và Điểm yếu Mạng}

\subsubsection{Rủi ro của Exposed Ports và Default Bridge Network}
Trong cấu hình mặc định, chúng tôi đã xác định các rủi ro nghiêm trọng sau:
\begin{itemize}
    \item \textbf{Exposed Ports:} Việc ánh xạ cổng của dịch vụ nội bộ (ML Worker) ra máy chủ vật lý (ví dụ: \texttt{-p 5000:5000}) tạo ra lỗ hổng Critical. Điều này cho phép kẻ tấn công truy cập trực tiếp vào worker, bỏ qua hoàn toàn lớp xác thực tại Gateway.
    \item \textbf{Default Bridge Network:} Mạng cầu nối mặc định của Docker không cung cấp tính năng phân giải DNS tự động và quan trọng hơn là thiếu sự cô lập (isolation). Tất cả các container trên mạng này có thể giao tiếp tự do với nhau, tạo điều kiện cho tấn công leo thang (lateral movement).
\end{itemize}

\subsubsection{Điểm yếu trong Overlay và MACVLAN Networks}
\begin{itemize}
    \item \textbf{Overlay Networks:} Mặc dù hỗ trợ đa máy chủ (Swarm), mạng Overlay mặc định không mã hóa lưu lượng (VXLAN traffic), yêu cầu quản lý khóa phức tạp và mở thêm các cổng quản lý (2377, 7946, 4789), làm tăng bề mặt tấn công.
    \item \textbf{MACVLAN Networks:} Cung cấp hiệu năng cao nhưng gặp vấn đề về cạn kiệt địa chỉ MAC (MAC address exhaustion) và yêu cầu chế độ promiscuous trên card mạng vật lý, điều này thường bị cấm trong môi trường Cloud hoặc Enterprise nghiêm ngặt.
\end{itemize}

\subsection{Các Kịch bản Tấn công Tiềm năng}

\begin{enumerate}
    \item \textbf{ARP Spoofing (Man-in-the-Middle):} Kẻ tấn công chiếm quyền kiểm soát một container trong cùng mạng có thể giả mạo gói tin ARP để chuyển hướng lưu lượng giữa Gateway và Worker qua máy của chúng nhằm đánh cắp dữ liệu.
    \item \textbf{Network Sniffing:} Do giao tiếp giữa các container mặc định là HTTP (plaintext), kẻ tấn công có thể sử dụng \texttt{tcpdump} để bắt gói tin và đọc toàn bộ nội dung nhạy cảm.
    \item \textbf{Misconfiguration Exploitation:} Lợi dụng việc cấu hình sai (như expose port nội bộ), kẻ tấn công có thể thực hiện DoS hoặc khai thác lỗ hổng ứng dụng trực tiếp từ internet.
\end{enumerate}

\subsection{Kết quả Kiểm thử Bảo mật Thực hành (Hands-on Security Tests)}

Chúng tôi đã thực hiện 3 bài kiểm tra bảo mật thực tế trên môi trường \texttt{test-net}.

\subsubsection{Test 1: Port Scanning từ Container nội bộ}
\textbf{Mục tiêu:} Kiểm tra khả năng khám phá dịch vụ (Service Discovery) của kẻ tấn công.
\textbf{Thực hiện:} Sử dụng công cụ \texttt{nmap} từ một container Alpine độc hại.
\textbf{Kết quả:}
\begin{verbatim}
...
5000/tcp open  upnp
\end{verbatim}
\textbf{Đánh giá:} Kẻ tấn công có thể lập bản đồ toàn bộ hệ thống mạng trong vòng chưa đầy 1 giây do thiếu các quy tắc tường lửa nội bộ.

\subsubsection{Test 2: Network Packet Capture (Sniffing)}
\textbf{Mục tiêu:} Chứng minh rủi ro của việc truyền tải dữ liệu không mã hóa.
\textbf{Thực hiện:} Sử dụng container \texttt{netshoot} chạy \texttt{tcpdump} để bắt gói tin giữa Gateway và Worker.
\textbf{Kết quả:} Bắt được toàn bộ payload JSON dưới dạng plaintext:
\begin{verbatim}
...
\end{verbatim}
\textbf{Đánh giá:} \textbf{CRITICAL}. Dữ liệu dự đoán và thông tin nghiệp vụ bị lộ hoàn toàn. Vi phạm các tiêu chuẩn bảo mật như PCI-DSS và GDPR.

\subsubsection{Test 3: Mô phỏng Misconfiguration (Exposed Internal Port)}
\textbf{Mục tiêu:} Giả lập lỗi cấu hình phổ biến khi developer expose port của backend service.
\textbf{Thực hiện:} Chạy worker với flag \texttt{-p 5000:5000} và thử truy cập trực tiếp bỏ qua Gateway.
\textbf{Lệnh tấn công:} \texttt{curl -X POST http://localhost:5000/predict}
\textbf{Kết quả:} Truy cập thành công và nhận được kết quả dự đoán mà không cần xác thực qua Gateway.
\textbf{Đánh giá:} Lỗ hổng này phá vỡ hoàn toàn kiến trúc bảo mật phân tầng.

\subsection{Giải pháp Bảo mật Đề xuất}

Dựa trên kết quả đánh giá, các biện pháp sau cần được áp dụng:

\begin{enumerate}
    \item \textbf{Mã hóa đường truyền (TLS/mTLS):} Triển khai Mutual TLS để mã hóa giao tiếp giữa Gateway và Worker, ngăn chặn Sniffing và Spoofing.
    \item \textbf{Phân đoạn mạng (Network Segmentation):} Tách biệt mạng Frontend (Public) và Backend (Internal). Worker chỉ nên nằm trong mạng Backend và không được expose port ra host.
    \item \textbf{Access Control \& Firewalls:} Sử dụng \texttt{iptables} hoặc Docker Network Policies để chỉ cho phép Gateway giao tiếp với Worker trên cổng 5000, chặn tất cả các kết nối khác.
    \item \textbf{Least Privilege:} Chạy container với user non-root và hạn chế quyền (drop capabilities) như trong file \texttt{Dockerfile.worker.secure}.
\end{enumerate}
...
% \section{Task 5 -- Monitoring and Observability}

\subsection{Mục tiêu}

Trong Task 5, nhóm tập trung quan sát và đánh giá hệ thống đã xây dựng ở các task trước (container API trên Machine A giao tiếp với container worker trên Machine B bằng \textbf{Method 1 -- port mapping}). Mục tiêu chính gồm:

\begin{itemize}
    \item Theo dõi \textbf{metrics của container} (CPU, RAM, network I/O).
    \item Theo dõi \textbf{traffic mạng} giữa API và worker.
    \item Trực quan hoá ít nhất \textbf{hai loại dữ liệu}.
    \item Phát hiện tối thiểu \textbf{một vấn đề/bottleneck} dựa trên kết quả monitoring.
\end{itemize}

Trong phạm vi bài lab, nhóm lựa chọn kết hợp hai nhóm công cụ nhẹ, dễ triển khai:

\begin{itemize}
    \item \textbf{Docker stats / Docker logs}: theo dõi resource usage và log ứng dụng ở mức container.
    \item \textbf{tcpdump + Wireshark}: thu thập và phân tích traffic mạng giữa hai máy/container.
\end{itemize}
...
%--------------------------------------------------------
\subsection{Kiến trúc và công cụ giám sát}

Hệ thống được giám sát trong Task 5 gồm:

\begin{itemize}
    \item \textbf{Machine A}:
    \begin{itemize}
        \item Container \texttt{api-gateway}, publish cổng \texttt{8000}.
        \item Nhận request từ client và gửi HTTP request tới worker trên Machine B.
    \end{itemize}
    \item \textbf{Machine B}:
    \begin{itemize}
        \item Container \texttt{ml-worker}, publish cổng \texttt{5000}.
        \item Xử lý request (giả lập suy luận mô hình) và trả kết quả cho API.
    \end{itemize}
\end{itemize}

Các công cụ monitoring được sử dụng cụ thể như sau:

\paragraph{Docker stats / Docker logs}

Trong thời gian chạy benchmark, nhóm sử dụng các lệnh:
...
\begin{itemize}
    \item \texttt{docker stats} cung cấp các chỉ số:
    \begin{itemize}
        \item CPU\% theo thời gian.
        \item Memory usage của từng container.
        \item Network I/O (bytes sent/received).
    \end{itemize}
    \item \texttt{docker logs} hỗ trợ:
    \begin{itemize}
        \item Ghi nhận lỗi HTTP (timeout, mã lỗi 5xx).
        \item Kiểm tra thời gian xử lý mỗi request (nếu ứng dụng có log).
    \end{itemize}
\end{itemize}

\paragraph{tcpdump + Wireshark}

Để quan sát traffic mạng khi tải tăng, nhóm sử dụng \texttt{tcpdump} trên host:

\begin{verbatim}
sudo tcpdump -i eth0 port 8000 -w method1_traffic.pcap
\end{verbatim}

File \texttt{.pcap} thu được được mở bằng \textbf{Wireshark} để:

\begin{itemize}
    \item Xem số lượng gói tin HTTP/TCP theo thời gian khi chạy benchmark.
    \item Quan sát round-trip time (RTT) của các HTTP request mẫu.
    \item Kiểm tra có gói tin bị \textit{retransmit} hoặc \textit{RST/FIN} bất thường hay không.
\end{itemize}

Trong báo cáo, nhóm chèn các hình minh hoạ:

\begin{itemize}
    \item Hình 5.1: Biểu đồ CPU và Net I/O của hai container từ \texttt{docker stats}.
    \item Hình 5.2: Biểu đồ lưu lượng HTTP theo thời gian trong Wireshark (filter theo \texttt{tcp.port == 8000} hoặc \texttt{5000}).
\end{itemize}

%--------------------------------------------------------
\subsection{Dữ liệu được quan sát và trực quan hoá}

Nhóm tập trung vào \textbf{hai loại dữ liệu chính} để trực quan hoá và phân tích:

\subsubsection{Resource metrics của container}

Từ \texttt{docker stats}, trong khi chạy công cụ benchmark (ví dụ \texttt{wrk} với \texttt{2 threads}, \texttt{20 connections}, thời gian khoảng \texttt{30s}), nhóm quan sát được:

\begin{itemize}
    \item \textbf{Container \texttt{api-gateway}}:
    \begin{itemize}
        \item CPU dao động quanh \textit{X--Y\%} khi số lượng request đồng thời tăng.
        \item Memory ổn định quanh \textit{M} MB, không có xu hướng tăng dần (không phát hiện dấu hiệu memory leak).
        \item Net I/O tăng đều theo thời gian, phù hợp với số lượng request/response được sinh ra.
    \end{itemize}
    \item \textbf{Container \texttt{ml-worker}}:
    \begin{itemize}
        \item CPU thường cao hơn \texttt{api-gateway} do chịu tải xử lý chính, có thời điểm lên tới khoảng \textit{E--F\%} khi tăng concurrency.
        \item Memory ổn định ở mức \textit{N} MB.
        \item Net I/O phản ánh pattern nhận request từ API (RX) và trả kết quả (TX).
    \end{itemize}
\end{itemize}
...
\subsubsection{Network traffic và latency ở tầng gói tin}

Từ file \texttt{method1\_traffic.pcap} (mở bằng Wireshark), nhóm quan sát:

\begin{itemize}
    \item Số lượng gói tin TCP/HTTP theo thời gian trong giai đoạn chạy benchmark.
    \item Thời gian đáp ứng (khoảng thời gian giữa request và response) cho một số request mẫu.
    \item Sự xuất hiện (hoặc không xuất hiện) của các gói \textit{TCP Retransmission}, \textit{Duplicate ACK}, \textit{RST}, v.v.
\end{itemize}

Biểu đồ traffic lọc theo \texttt{tcp.port == 8000} cho thấy:

\begin{itemize}
    \item Lưu lượng tăng rõ rệt trong khoảng thời gian benchmark.
    \item Không có burst bất thường ngoài thời gian test; traffic tương đối đều và ổn định.
\end{itemize}
...
%--------------------------------------------------------
\subsection{Vấn đề được phát hiện nhờ monitoring}

Bằng việc kết hợp \texttt{docker stats}, \texttt{docker logs} và phân tích Wireshark, nhóm phát hiện một bottleneck quan trọng khi tăng tải:

\begin{itemize}
    \item Khi tăng mức độ concurrency (ví dụ từ 20 lên 50 request đồng thời), metrics cho thấy:
    \begin{itemize}
        \item CPU của container \texttt{ml-worker} thường xuyên chạm ngưỡng cao (xấp xỉ \textit{E--F\%}), trong khi \texttt{api-gateway} vẫn còn dư tài nguyên.
        \item Latency trung bình và P95 từ công cụ benchmark tăng đáng kể (ví dụ P95 tăng từ khoảng \textit{L\_1 ms} lên \textit{L\_2 ms}).
        \item Trong log xuất hiện một số request timeout hoặc lỗi kết nối khi tải cao (nếu có).
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Không thấy pattern mất gói (packet loss) hay TCP retransmission đáng kể.
    \item Phần lớn request đều được gửi/nhận bình thường, nhưng thời gian đáp ứng tăng lên khi \texttt{ml-worker} bị bão hoà CPU.
\end{itemize}

Từ các quan sát này, nhóm kết luận:

\begin{itemize}
    \item Bottleneck chính nằm ở \textbf{khả năng xử lý của ứng dụng trong container \texttt{ml-worker}}, không phải do giới hạn của mạng:
    \begin{itemize}
        \item Ping giữa hai máy ổn định, không có packet loss đáng kể.
        \item Wireshark không chỉ ra hiện tượng tắc nghẽn hay lỗi truyền dẫn bất thường.
    \end{itemize}
    \item Để mở rộng hệ thống, giải pháp phù hợp không phải là thay đổi phương thức networking, mà là:
    \begin{itemize}
        \item Tối ưu logic xử lý trong \texttt{ml-worker}.
        \item Tăng tài nguyên cho Machine B (CPU/RAM).
        \item Hoặc scale-out nhiều replica \texttt{ml-worker} kết hợp load balancing.
    \end{itemize}
\end{itemize}

%--------------------------------------------------------
\subsection{Nhận xét và bài học kinh nghiệm}

Qua Task 5, nhóm rút ra một số bài học quan trọng:

\begin{itemize}
    \item Monitoring không chỉ là “xem CPU cho vui”, mà giúp \textbf{định vị chính xác nguyên nhân} khi hệ thống chậm:
    \begin{itemize}
        \item Khi kết hợp CPU, Net I/O, latency và traffic, nhóm có thể tránh phán đoán cảm tính.
    \end{itemize}
    \item Các công cụ nhẹ như \texttt{docker stats} / \texttt{docker logs} kết hợp \texttt{tcpdump + Wireshark} đã đủ hữu ích cho một bài lab nhỏ:
    \begin{itemize}
        \item Không cần ngay lập tức dựng Prometheus + Grafana nhưng vẫn quan sát được hành vi hệ thống.
    \end{itemize}
    \item Việc chụp lại các màn hình (dashboard metrics, biểu đồ traffic) là rất quan trọng:
    \begin{itemize}
        \item Minh chứng rằng kết luận được rút ra dựa trên số liệu thực tế.
        \item Tạo cơ sở để so sánh với các phương thức networking khác (overlay, VPN, message broker, \ldots) ở các task tiếp theo.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Thu thập time series metrics tự động cho nhiều container và nhiều node.
    \item Xây dựng dashboard tập trung để theo dõi toàn bộ hệ thống.
    \item Đặt alert khi CPU, latency hoặc error rate vượt ngưỡng, giống cách vận hành hệ thống trong môi trường production.
\end{itemize}
